...
export function CardProducts({ img, name, price, slug, colors, variants }: Props) {
  const [activeColor, setActiveColor] = useState<{name: string, color:string}>(colors[0])

  // Identificar la variante seleccionada segun el color activo(activeColor)
  ...
  return (
    <div className="flex flex-col gap-6 relative">
      <Link to={`/smarthphones/${slug}`} className="flex relative group overflow-hidden">
        ...
        <button className="bg-white border border-slate-200 absolute w-full bottom-0 py-3 rounded-3xl flex items-center justify-center gap-1 text-sm font-medium hover:bg-stone-100 translate-y-[100%] transition-all duration-300 group-hover:translate-y-0 cursor-pointer">
          <FiPlus />
          Añadir
        </button>
      </Link>
