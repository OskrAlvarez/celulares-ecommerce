...
import { socialLinks } from "@/common/constants/links";
    ...
    <footer className="py-16 bg-gray-950 px-12 flex justify-between gap-10 text-slate-200 text-sm flex-wrap md:flex-nowrap">
      ...
      <div className="flex flex-col gap-4 flex-1">
        ...
        <nav className="flex flex-col gap-2 text-xs font-medium">
          <Link to="/smartphones">Productos</Link>
        ...
        <div className="flex">
          {socialLinks.map(link => (
            <a key={link.id}
							...
							target='_blank'
							rel='noreferrer'
							className='text-slate-300 border border-gray-8000 w-full h-full py-3.5 flex items-center justify-center transition-all hover:bg-white hover:text-gray-950'>{link.icon}</a>
          ))}
        </div>
      </div>
    </footer>
